...
TEST_CASE("innerProduct operation on vector", "[innerProduct]") {
  ...
  REQUIRE(result1 == std::conj(result2)); // by mathematical definition
...
TEST_CASE("innerProduct operation on real matrix and ComplexMatrix",
          "[innerProduct]") {
...
TEST_CASE("innerProduct operation on real vector and ComplexVector",
          "[innerProduct]") {
  sclMath::RealMatrix m1(3, 1);
  sclMath::ComplexMatrix m2(3, 1);
